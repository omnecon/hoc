...
@Component({
    selector: 'jhi-image-my-suffix',
    templateUrl: './image-my-suffix.component.html'
})
export class ImageMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    images: ImageMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/image-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/image-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ImageMySuffix) {
        return item.id;
    }
