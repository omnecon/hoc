...
@Component({
    selector: 'jhi-banner-my-suffix',
    templateUrl: './banner-my-suffix.component.html'
})
export class BannerMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    banners: BannerMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/banner-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/banner-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: BannerMySuffix) {
        return item.id;
    }
