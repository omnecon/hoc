...
@Component({
    selector: 'jhi-gallery-my-suffix',
    templateUrl: './gallery-my-suffix.component.html'
})
export class GalleryMySuffixComponent implements OnInit, OnDestroy {

currentAccount: any;
    galleries: GalleryMySuffix[];
    error: any;
    success: any;
    ...
    transition() {
        this.router.navigate(['/gallery-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/gallery-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: GalleryMySuffix) {
        return item.id;
    }
